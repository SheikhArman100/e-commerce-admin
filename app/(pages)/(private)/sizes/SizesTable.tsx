...
export default function SizesTable() {
  ...
  const { data: sizesData, isLoading, error } = useSizes(filters);
  ...
  const sizes: ISize[] = sizesData?.data || [];
  const totalCount = sizesData?.meta?.count || 0;
                  ...
                  <TableRow key={size.id}>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        <span className={`w-2 h-2 rounded-full ${getStatusColor(size.isActive ? 'ACTIVE' : 'INACTIVE')}`}></span>
                        <span className="text-sm text-muted-foreground">{size.isActive ? 'Active' : 'Inactive'}</span>
                      </div>
                    </TableCell>
                    <TableCell className="">
