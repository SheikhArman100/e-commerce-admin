...
export const useCustomerInsights = (filters: DashboardFilters = {}) => {
  const axiosPrivate = useAxiosPrivate();
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await axiosPrivate.get<{ data: CustomerInsightsResponse }>(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/dashboard/customer-analytics?${params.toString()}`
      );
      return response.data.data;
    },
    enabled: true,
